//ALL COMPUTATIONS ARE ITERATIVE
/*
Name: Nikolas Novak
MacID: novakn
Student Number: 1406602
Description: This is a java program that receives as input a specifically formatted set of arguments from the command line which represent the properties and elements of a set of matrices. The program then attempts to multiply all of the matrices together, and compute the inverse of the result. The program notifies the user if any of these tasks is impossible.
*/
...
public class HWK2_novakn {//Beginning of HWK2_novakn class
	public static void main(String[] args){//Main method, with args passed in
		...
		if(input.length!=0){//perform the required operations only if there actually was input
			...
			double[][] result = invert(multM);//printM(invert(multM)); //Inverts the multiplied array and stores it in result
			for(int i=0;i!=result.length;i++)//for every row
				for(int j=0;j!=result[0].length;j++)//for every column
					System.out.printf("%.4f" + "\t \t",result[i][j]);//print the elements of the matrix row by row to 4 decimal places and separated by 2 tab characters
		}//end input if
